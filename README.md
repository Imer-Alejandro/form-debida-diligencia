...
Plataforma de registro de proveedores con formulario de debida diligencia (bilingüe ES/EN), panel de administración con evaluación interna, y subida de documentos a Microsoft OneDrive. Construida con Next.js 16 (App Router + Turbopack), Supabase y Tailwind CSS.

## Requisitos

- Node.js 22+ / 24+
- Proyecto Supabase (base de datos + autenticación de administradores)
- (Opcional) Registro de aplicación en Microsoft Entra para subidas a OneDrive
...
- `src/app/` — páginas (App Router)
  ...
  - `/admin` — panel: dashboard, proveedores, invitaciones, configuración
  - `/api/admin/*` — mutaciones del panel (evaluación, tags, invitaciones, cambio de datos)
  - `/api/upload/*` y `/api/onedrive/*` — subida de documentos y conexión con OneDrive
...
- `src/lib/onedrive.ts`, `src/lib/supabase/` — integración con OneDrive y Supabase
...
| `ONEDRIVE_CLIENT_ID` | Client ID del app registration (solo subidas a OneDrive) |
| `ONEDRIVE_TENANT_ID` | Tenant ID o `common` |
...
2. El proveedor abre `/i/[token]`, completa el formulario en dos idiomas, adjunta los documentos (se suben directo a OneDrive) y los firma.
