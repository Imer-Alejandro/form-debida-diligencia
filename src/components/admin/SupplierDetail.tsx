...
export function SupplierDetail({
  ...
  return (
    <div>
      ...
      <div className="mt-3 rounded-2xl border border-navy-800/10 bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[13px] text-ink-muted">
              <span className="tabular-nums">{reg.reference_no}</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge tone={riskTone(reg.risk_level)}>{t(`risk.${reg.risk_level}` as never)}</Badge>
            <Badge tone={statusTone(reg.status)}>{t(`statuses.${reg.status}` as never)}</Badge>
          </div>
        </div>
        ...
        <div className="mt-5 border-t border-navy-800/10 pt-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1.5 rounded-full bg-navy-800/10 px-3 py-1 text-[12.5px] text-ink-soft"
              >
                {tag}
                <button
